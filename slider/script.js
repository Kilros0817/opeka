...
/* ─────────────────────────────────────────────────────────────
   ...
   – draggable works without InertiaPlugin (uses plain Draggable)
───────────────────────────────────────────────────────────── */
function horizontalLoop(items, config) {
  ...
  gsap.context(() => {
    ...
    /* ── Draggable (no InertiaPlugin needed) ── */
    if (config.draggable && typeof Draggable === "function") {
      proxy = document.createElement("div");
      let startProgress, dragRatio, wasPlaying;

      const draggableInst = Draggable.create(proxy, {
        ...
        onDrag() {
          tl.progress(
            gsap.utils.wrap(
              0,
              1,
              startProgress + (this.startX - this.x) * dragRatio
            )
          );
        },
        onRelease() {
          ...
          indexIsDirty = false;
          let snapTime =
            times[getClosest(times, tl.time(), tl.duration())];
          gsap.to(tl, {
            time: snapTime,
            duration: 0.4,
            ease: "power1.out",
            overwrite: true,
            onComplete: () => wasPlaying && tl.play()
          });
        }
      })[0];
